...
public sealed class PayjoinInvoicePaymentUrlService : IPayjoinInvoicePaymentUrlService
{
    ...
    private static bool IsPayjoinEnabled(string paymentUrl)
    {
        ...
        catch (Exception ex) when (PayjoinExceptionCompatibility.IsUnsupportedException(ex))
        {
            return false;
        }
        catch (Exception ex) when (PayjoinExceptionCompatibility.IsParseException(ex))
        {
            return false;
        }
    }
